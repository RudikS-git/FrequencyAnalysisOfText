...
const History = () => {
    ...
    const deleteHistoryItem = async (id) => {
        const { success, data } = await doRequest(axios.delete(`/?id=${id}`));
    ...
    const downloadFile = async (id) => {
        const { success, data } = await doRequest(axios.get(`download-analysis-file/?id=${id}`, { responseType: 'blob' }));
                            ...
                            <tbody>
                            {
                                items && items?.map(it => {
                                    return (
                                        <tr>
                                            <td>{it.textName}</td>
                                            <td>{it.amountSymbols}</td>
                                            <td>{new Date(Date.parse(it.dateAnalysis)).toLocaleString("ru-RU")}</td>
                                            <td>
